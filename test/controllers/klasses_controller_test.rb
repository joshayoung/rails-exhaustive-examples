...
class KlassesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create klass" do
    assert_difference('Klass.count') do
      post klasses_url, params: { klass: { level: @klass.level, name: @klass.name, time: @klass.time } }
    end
  ...
  test "should update klass" do
    patch klass_url(@klass), params: { klass: { level: @klass.level, name: @klass.name, time: @klass.time } }
